...
ui <- navbarPage(
  ...
  tabPanel("About",
           ...
           HTML("<p></p>
                ...
                <p>Note that there are many other vaccine appointments in Massachusetts which are not covered by @vaccinetime (and so this app). They include appointments at the seven mass vaccination centers now handled by the state's <a target='_new' href='https://vaccinesignup.mass.gov/#/'>central registration system</a>, health organizations such as Mass General Brigham, and some locally run sites, among others. In addition, while CVS appointment availability is tweeted, CVS does not post the exact number of appointments they have available each week, so they aren't included in totals.</p>
